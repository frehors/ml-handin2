...
def softmax(X):
    ...
    for row in range(X.shape[0]):
        max_x = np.max(X[row])
        logsum = np.log(np.sum(np.exp(X[row] - max_x))) + max_x
...
class NetClassifier():
    ...
    def predict(self, X, params=None):
        ...
        sm = softmax(np.dot(h, params['W2']) + params['b2'])
    ...
    def cost_grad(X, y, params, c=0.0):
        ...
        #h = np.hstack((h, np.ones((h.shape[0], 1))))
        print('h', h.shape)
        sm = softmax(np.dot(h, W2) + b2)
        cost = -np.sum(labels * np.log(sm)) / X.shape[0] + c * (np.sum(W1 * W1) + np.sum(W2 * W2))

        ### YOUR CODE HERE - BACKWARD PASS - compute gradients
        d_w2 = np.dot(h.T, sm - labels) / X.shape[0] + 2 * c * W2
        d_b2 = np.sum(sm - labels, axis=0) / X.shape[0]
        d_h = np.dot(sm - labels, W2.T)
        #d_h = d_h[:, :-1]   # Remove bias
        d_h[h <= 0] = 0
        d_w1 = np.dot(X.T, d_h) / X.shape[0] + 2 * c * W1
        d_b1 = np.sum(d_h, axis=0) / X.shape[0]
